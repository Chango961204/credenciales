...
router.post("/", /* protect, authorize("admin"), */ async (req, res) => {
  try {
    ...
    if (!frente || !reverso) {
      return res.status(400).json({ error: "Se requieren ambas imágenes: frente y reverso" });
    }
    await printDoubleSidedCard(frente, reverso, { printerName: "Zebra ZXP Series 3 USB Card Printer" });
